...
psql "dbname=mimic user='$POSTGRES_USER' options=--search_path=mimiciii" < /mimic-code/postgres_create_tables_pg10.sql
else
echo "$0: running postgres_create_tables_pg.sql"
psql "dbname=mimic user='$POSTGRES_USER' options=--search_path=mimiciii" < /mimic-code/postgres_create_tables_pg.sql
fi

if [ $COMPRESSED -eq 1 ]; then
echo "$0: running postgres_load_data_gz.sql"
psql "dbname=mimic user='$POSTGRES_USER' options=--search_path=mimiciii" -v mimic_data_dir=/mimic_data < /mimic-code/postgres_load_data_gz.sql
else
echo "$0: running postgres_load_data.sql"
psql "dbname=mimic user='$POSTGRES_USER' options=--search_path=mimiciii" -v mimic_data_dir=/mimic_data < /mimic-code/postgres_load_data.sql
fi

echo "$0: running postgres_add_indexes.sql"
psql "dbname=mimic user='$POSTGRES_USER' options=--search_path=mimiciii" < /mimic-code/postgres_add_indexes.sql

echo "$0: running postgres_add_constraints.sql"
psql "dbname=mimic user='$POSTGRES_USER' options=--search_path=mimiciii" < /mimic-code/postgres_add_constraints.sql


echo "$0: running postgres_checks.sql (all rows should return PASSED)"
psql "dbname=mimic user='$POSTGRES_USER' options=--search_path=mimiciii" < /mimic-code/postgres_checks.sql
fi
